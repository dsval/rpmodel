...
rpmodel.grid<-function(tc, vpd, co2, fapar, ppfd, elev = NA,AI=NA ,
	kphio = ifelse(do_ftemp_kphio, calc_phi0(tc=tc,AI=AI), 0.0817), 
	...
	parallel::clusterExport(cl, varlist=c("tc","vpd","co2",'elev','fapar',"ppfd","kphio",'beta','soilm','meanalpha','apar_soilm','bpar_soilm','c4','method_optci','method_jmaxlim','do_ftemp_kphio','do_soilmstress','ny','y','bs'),envir=environment()) 
	pb <- pbCreate(bs$n)
	...
	clFun <- function(i) {
		...
		calc_transp<-function(mat,vpd,tc,ppfd ,elev){
			...
			gs_mol<-as.numeric(mat$gs)*press
			gs_mol[gs_mol<0]<-NA
			T_mol<-1.6*gs_mol*(vpd)/press
